...
def build_model():
    m_rock = create_pbr_material("M_Rock_Granite", base_color=(0.35, 0.35, 0.38, 1.0), roughness=0.92)
    m_moss = create_pbr_material("M_Rock_MossVelvet", base_color=(0.14, 0.38, 0.1, 1.0), roughness=0.65, subsurface=0.15)

    # Main Bounding Rock
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=3, radius=0.9, location=(0, 0, 0.55))
    rock = bpy.context.active_object
    rock.scale = (1.3, 0.95, 0.75)
    rock.name = "Cliff_Rock_Core"
    rock.data.materials.append(m_rock)

    # Moss Top Layer (Cap)
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2, radius=0.88, location=(0, 0, 0.72))
    moss = bpy.context.active_object
    moss.scale = (1.25, 0.9, 0.5)
    moss.name = "Rock_Moss_Cap"
    moss.data.materials.append(m_moss)

    # Side Boulders
    boulders = [(0.95, -0.3, 0.25, 0.35), (-0.85, 0.4, 0.22, 0.32), (0.2, 0.75, 0.28, 0.4)]
    for i, (bx, by, bz, brad) in enumerate(boulders):
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2, radius=brad, location=(bx, by, bz))
        b = bpy.context.active_object
        b.scale = (1.1, 0.9, 0.8)
        b.name = f"Rock_Boulder_{i}"
        b.data.materials.append(m_rock)
...
def main():
    ...
    glb_path = os.path.join(assets_dir, "mossy_cliff_rock.glb")
    ...
    # 4-Cut Renders
    render_cut(os.path.join(assets_dir, "mossy_cliff_rock_cut1.png"), (2.2, -3.0, 2.0), (65, 0, 35), 50)
    render_cut(os.path.join(assets_dir, "mossy_cliff_rock.png"), (2.2, -3.0, 2.0), (65, 0, 35), 50)
    render_cut(os.path.join(assets_dir, "mossy_cliff_rock_cut2.png"), (0.8, -1.6, 1.8), (70, 0, 22), 75)
    render_cut(os.path.join(assets_dir, "mossy_cliff_rock_cut3.png"), (1.8, -2.0, 3.2), (48, 0, 38), 45)
    render_cut(os.path.join(assets_dir, "mossy_cliff_rock_cut4.png"), (2.0, -1.8, 0.5), (82, 0, 48), 35)
