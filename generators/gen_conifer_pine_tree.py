...
def build_model():
    m_bark = create_pbr_material("M_Pine_Bark", base_color=(0.22, 0.14, 0.09, 1.0), roughness=0.88)
    m_needle = create_pbr_material("M_Pine_Needles", base_color=(0.06, 0.2, 0.08, 1.0), roughness=0.55, subsurface=0.18)

    # Trunk
    bpy.ops.mesh.primitive_cylinder_add(vertices=12, radius=0.16, depth=3.2, location=(0, 0, 1.6))
    trunk = bpy.context.active_object
    trunk.scale = (1.0, 1.0, 1.0)
    trunk.name = "Pine_Trunk"
    trunk.data.materials.append(m_bark)

    # 5 Tiers of Needle Cones
    tiers = [
        (1.1, 0.85, 1.35),
        (0.92, 0.8, 1.9),
        (0.74, 0.75, 2.45),
    ...
    for i, (r, h, z) in enumerate(tiers):
        bpy.ops.mesh.primitive_cone_add(vertices=10, radius1=r, depth=h, location=(0, 0, z))
        cone = bpy.context.active_object
        cone.rotation_euler = (0, 0, math.radians(i * 24))
        cone.name = f"Pine_Foliage_Tier_{i}"
        cone.data.materials.append(m_needle)
...
def main():
    ...
    glb_path = os.path.join(assets_dir, "conifer_pine_tree.glb")
    ...
    # 4-Cut Renders
    render_cut(os.path.join(assets_dir, "conifer_pine_tree_cut1.png"), (2.8, -4.2, 2.6), (65, 0, 35), 50)
    render_cut(os.path.join(assets_dir, "conifer_pine_tree.png"), (2.8, -4.2, 2.6), (65, 0, 35), 50)
    render_cut(os.path.join(assets_dir, "conifer_pine_tree_cut2.png"), (0.8, -1.6, 1.8), (70, 0, 22), 75)
    render_cut(os.path.join(assets_dir, "conifer_pine_tree_cut3.png"), (1.8, -2.0, 3.2), (48, 0, 38), 45)
    render_cut(os.path.join(assets_dir, "conifer_pine_tree_cut4.png"), (2.0, -1.8, 0.5), (82, 0, 48), 35)
