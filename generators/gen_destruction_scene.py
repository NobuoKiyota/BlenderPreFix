...
def setup_explosion_effects(hit_pos, spark_mat, fire_smoke_mat, custom_params=None):
    ...
    if enable_mantaflow:
        ...
        # 炎源メッシュは既定の灰色マテリアルの不透明球のままレンダー/ビューポート
        # に映り込み、ちょうど炎の中心に重なって炎を覆い隠してしまう
        # (実機検証で確認済みの罠)。Flow役のメッシュはレンダーに映す必要がない。
        ...
        # キャッシュ先を絶対パスに固定する(既定の相対パス "//.." は、この
        # シーンがまだ保存されていない時点(ベイクはsave_as_mainfile()より前)
        # では解決できず、キャッシュが正しく書き出されないことがあるため)。
        cache_dir = os.path.join(
            tempfile.gettempdir(), "blenderprefix_fluid_cache", uuid.uuid4().hex
        )
